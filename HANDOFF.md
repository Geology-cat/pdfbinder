...
3. 「PDFを作成」→ `NSSavePanel` → バックグラウンドで合成・書き出し → Finderで保存先を表示
...
2. **画像ページサイズの正規化オプション**: 現状は画像のピクセルサイズがページサイズになる。高解像度写真だと巨大なページになるため、「A4に収める／原寸」の選択肢を追加するのが望ましい（`PDFComposer` に描画ベースの実装を追加。`NSImage` を `CGContext` でA4矩形にアスペクト維持で描画）
3. **書き出し完了のフィードバック改善**: 現在はFinder表示のみ。トースト等の検討
...
9. **大量ファイル対策**: プレビューの差分更新、`PDFDocument` 合成の進捗表示、サムネイルの`NSCache`化
